		...
		default:
			{
				...
				m_aFBO[uiType]->GLCreateFrameBuffer();
				m_aFBO[uiType]->GLBind();
				{
					if (!m_aFBO[uiType]->GLCreateColorTexture2D(GL_COLOR_ATTACHMENT0, width, height) 
						|| !m_aFBO[uiType]->GLCreateDepthStencilRender(width, height) 
